...
def tst_model_latency(model, device, example_inputs, iterations=1000):
    """测试模型的推理延迟。"""
    logging.info("Tsting latency...")
    time_arr = []
    for _ in tqdm(range(iterations), desc='Latency Tst'):
        if device.type == 'cuda':
...
def main(opt):
    ...
    mean_latency, std_latency = tst_model_latency(model, device, example_inputs, opt.tsttime)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Tst YOLOv8 model performance.")
    parser.add_argument('--weights', type=str, default=r'E:\Desktop\YOLOv8\runs\train\exp57\weights\best.pt', help='trained weights path')
    ...
    parser.add_argument('--tsttime', default=1000, type=int, help='tst iterations for latency')
